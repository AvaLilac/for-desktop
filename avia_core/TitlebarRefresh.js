(function () {
  ...
  function inject() {
    const titlebars = document.querySelectorAll(
      ".flex-sh_0.h_29px.us_none.d_flex.ai_center"
    );

    titlebars.forEach(titlebar => {
      if (titlebar.querySelector(".avia-refresh-button")) return;

      const buttons = [...titlebar.children].filter(
        el => el.tagName === "A"
      );
      ...
      button.className =
        "avia-refresh-button cursor_pointer pos_relative d_grid place-items_center h_100% asp_3/2";
      ...
      if (minimizeButton) {
        titlebar.insertBefore(button, minimizeButton);
      }
    });
  }
